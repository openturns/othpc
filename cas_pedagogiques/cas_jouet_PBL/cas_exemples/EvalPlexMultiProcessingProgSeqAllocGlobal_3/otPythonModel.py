...
class funcPythonCode(ot.OpenTURNSPythonFunction):
    ...
    def run_myProgram(self,resultDir, dictInputs, restartPath=None):
        """
        ...
        command.append(os.path.join(os.path.abspath(self.binPath), "myProgram"))
        ...
        command.append("--cpu-interval=%d" % (1800))
    ...
    def _exec(self,X):
        ...
        check_run_id = tb.checkSuccessfullRun(
            case_path=case_path, file_basename=self.successfullRunFileName, **dict_inputs
        )

        # parse case_path/needRestart_run.csv to check if run point need a restart
        checkRestart_run_id = tb.checkSuccessfullRun(
            case_path=case_path, file_basename=self.needRestartRunFileName, **dict_inputs
        )
        ...
        if check_run_id == checkRestart_run_id and checkFailed_run_id == None:
            ...
            if checkRestart_run_id == None:
                r = self.run_myProgram(
                    os.path.join(case_path, "RESU", run_id), dictInputs=dict_inputs
                )
                newComputation = True
        ...
        if r == 0:
            ...
            needRestart = True
            if checkRestart_run_id != None:
                resultFileName = os.path.join(case_path, "RESU", check_run_id, "result.txt")
                resultFile = open(resultFileName, "r")
                lines = resultFile.readlines()
