    ...
    def _exec_sample(self, X):
        ...
        for job in jobs:
            if job.state == "COMPLETED":
                result_list.append(job.results())
            else:
                evaluation_error_log(
                    Exception(job.exception()),
                    os.getcwd(),
                    f"LikelyTimeout_{job.job_id}.txt",
                )
                result_list.append(
                    [[float("nan")] * self.getOutputDimension()]
                    * job.submission().args[0].getSize()
                )
        result = ot.Sample(concatenate(result_list, axis=0))
